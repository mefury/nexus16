...
impl TaskFetcher {
    ...
    pub async fn fetch_task(&mut self) -> Result<Task, FetchError> {
        loop {
            ...
            if can_proceed_immediately {
                // Calculate round-robin delay for this worker
                let round_robin_delay = self.calculate_round_robin_delay();

                if round_robin_delay > Duration::ZERO {
                    self.event_sender
                        .send_task_event(
                            format!(
                                "Step 1 of 4: Round-robin wait ({}ms) - coordinating with {} workers",
                                round_robin_delay.as_millis(),
                                self.total_workers
                            ),
                            EventType::Waiting,
                            LogLevel::Info,
                        ...
                        .await;
                    sleep(round_robin_delay).await;

                    // After round-robin delay, check rate limiting again
                    if !self.network_client.request_timer_mut().can_proceed() {
    ...
    /// Calculate round-robin delay for this worker to coordinate with others
    fn calculate_round_robin_delay(&self) -> Duration {
        if self.total_workers <= 1 {
            return Duration::ZERO; // No delay needed for single worker
        }

        // Calculate delay based on worker position in round-robin cycle
        // This ensures workers fetch in sequence: 0, 1, 2, 3, 4, 0, 1, 2...
        let cycle_position = self.worker_id % self.total_workers;
        let delay_ms = cycle_position as u64 * 2000; // 2 seconds between workers

        // Add small randomization to prevent exact synchronization
        let random_offset = ((self.worker_id * 47) % 100) as u64; // Small pseudo-random offset
        let total_delay_ms = delay_ms + random_offset;
