...
use crate::runtime::start_authenticated_workers_with_difficulty;
...
pub async fn setup_session(
    ...
    // Clamp the number of workers to [1,16].
    let num_workers: usize = max_threads.unwrap_or(1).clamp(1, 16) as usize;
    ...
    // Start authenticated workers (supports multi-worker mode now)
    let (event_receiver, join_handles, max_tasks_shutdown_sender) = start_authenticated_workers_with_difficulty(
        node_id,
        signing_key,
